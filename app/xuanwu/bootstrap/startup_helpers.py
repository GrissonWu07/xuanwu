...
def build_token_entries(config) -> tuple[list[TokenEntry], Optional[str]]:
    ...
    from app.xuanwu.models.providers import BUILTIN_PROVIDERS
    ...
    if not base_url:
        raise RuntimeError(
            ...
            f"Set environment variable or configure in xuanwu.json under model.providers.{provider}"
        )
    if not api_key:
        env_hint = f" or set {preset.env_key}" if preset and preset.env_key else ""
        raise RuntimeError(
            f"Missing api_key for provider '{provider}'. "
            f"Configure in xuanwu.json under model.providers.{provider}{env_hint}"
        )
...
async def load_agent_config_from_db(session, agent_id: str):
    ...
    from app.xuanwu.agent.agent_definition import AgentConfig
