...
def load_handler_from_file(
    ...
    try:
        ...
        module_name = f"xuanwu_md_skill_{module_hash}_{py_file.stem}"
...
def create_script_wrapper(py_file: Path, provider_type: Optional[str] = None) -> Callable:
    ...
    async def script_handler(ctx=None, **kwargs) -> dict:
        ...
        if ctx is not None and hasattr(ctx, "deps") and hasattr(ctx.deps, "cookies"):
            ...
            if cookies:
                try:
                    env["XUANWU_COOKIES"] = json.dumps(cookies)
                    if hasattr(ctx.deps, "user_info") and ctx.deps.user_info:
                        print(
                            f"[DEBUG] Set XUANWU_COOKIES for user={ctx.deps.user_info.user_id}, "
        ...
        if ctx is not None and hasattr(ctx, "deps") and hasattr(ctx.deps, "extra"):
            ...
            # Fallback for channel-triggered path where provider_config may be omitted.
            ...
            if provider_config:
                try:
                    env["XUANWU_PROVIDER_CONFIG"] = json.dumps(provider_config)
                    print(
                        "[DEBUG] Set XUANWU_PROVIDER_CONFIG with providers: "
