...
class WorkspaceConfig(BaseModel):
    ...
    path: str = Field(default="./.xuanwu", description="Workspace path, defaults to ./.xuanwu directory")


class SqliteDatabaseConfig(BaseModel):
    """SQLite database configuration."""
    path: str = Field(default="./data/xuanwu.db", description="Path to SQLite database file")
...
class MySqlDatabaseConfig(BaseModel):
    ...
    database: str = Field(default="xuanwu", description="Database name")
...
class XuanWuConfig(BaseModel):
    ...
    auth: Optional[Any] = Field(
        default=None,
        description="Authentication configuration mapped from the auth section of xuanwu.json; falls back to anonymous mode when missing",
    )
