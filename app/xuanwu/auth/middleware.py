...
def setup_auth_middleware(
    ...
    from app.xuanwu.auth.strategy import create_auth_strategy
    ...
    if strategy is None:
        ...
        app.add_middleware(
            ...
            strategy=AuthStrategy(
                providers=[
                    __import__(
                        "app.xuanwu.auth.providers.none", fromlist=["NoneProvider"]
                    ).NoneProvider()
                ],
                shadow_store=__import__(
                    "app.xuanwu.auth.shadow_store", fromlist=["ShadowUserStore"]
                ).ShadowUserStore(),
            ),
