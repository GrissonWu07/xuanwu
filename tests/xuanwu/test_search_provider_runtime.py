...
from app.xuanwu.tools.web.provider_runtime import (
...
def test_runtime_prefers_more_query_relevant_provider_results_over_first_non_empty_provider() -> None:
    registry = SearchProviderRegistry()
    first = _FakeAdapter({
        '明天上海天气': [
            NormalizedSearchResult(
                title='Download the YouTube mobile app',
                url='https://support.google.com/youtube/answer/3227660',
                snippet='Download the YouTube app for a richer viewing experience.',
    ...
    import asyncio
    response = asyncio.run(runtime.execute(query='明天上海天气', provider_override=None, limit=5))

    assert response.provider == 'google_html_fallback'
    assert response.results[0].title == '上海天气预报'
    assert first.calls == ['明天上海天气']
    assert second.calls == ['明天上海天气']


def test_runtime_does_not_keep_first_provider_when_second_provider_matches_more_query_terms() -> None:
    registry = SearchProviderRegistry()
    first = _FakeAdapter({
        '明天上海天气': [
            NormalizedSearchResult(
                title='上海旅游攻略',
                url='https://travel.example.com/shanghai-guide',
                snippet='上海旅游景点推荐与自由行攻略，四季出行建议。',
                provider='bing_html_fallback',
                rank=1,
            ),
            NormalizedSearchResult(
                title='游戏活动大全',
                url='https://games.example.com/event',
                snippet='明日大型活动与奖励说明。',
                provider='bing_html_fallback',
                rank=2,
            ),
...
def test_runtime_prefers_hiking_results_over_generic_city_reference_results() -> None:
    registry = SearchProviderRegistry()
    first = _FakeAdapter({
        '苏州徒步推荐': [
            NormalizedSearchResult(
                title='苏州 (江苏省辖地级市)_搜狗百科',
                url='https://baike.example.com/suzhou',
                snippet='苏州，简称苏，隶属于江苏省，东傍上海，南接浙江，西抱太湖。',
                provider='bing_html_fallback',
                rank=1,
            ),
            NormalizedSearchResult(
                title='太仓（江苏省苏州市太仓市）_百科',
                url='https://baike.example.com/taicang',
                snippet='太仓位于江苏省东南部，长江口南岸。',
                provider='bing_html_fallback',
                rank=2,
            ),
        ]
    ...
    second = _FakeAdapter({
        '苏州徒步推荐': [
            NormalizedSearchResult(
                title='苏州徒步路线推荐',
                url='https://travel.example.com/suzhou-hiking',
                snippet='推荐灵岩山、天平山、穹窿山等徒步路线，附难度与交通。',
                provider='google_html_fallback',
                rank=1,
            ),
        ]
    ...
    runtime = SearchExecutionRuntime(
        ...
        selector=SearchProviderSelector(registry),
        planner=SearchQueryPlanner(official_domains=[]),
        prioritizer=SourcePrioritizer(official_domains=[], trusted_domains=[]),
        default_provider='bing_html_fallback',
    )

    import asyncio
    response = asyncio.run(runtime.execute(query='苏州徒步推荐', provider_override=None, limit=5))

    assert response.provider == 'google_html_fallback'
    assert response.results[0].title == '苏州徒步路线推荐'
    assert first.calls == ['苏州徒步推荐']
    assert second.calls == ['苏州徒步推荐']
